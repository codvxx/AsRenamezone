...
async def add_prefix_handler(client, message):
    ...
    if len(message.command) == 1:
        return await message.reply_text("**__Give The Prefix__\n\nExᴀᴍᴩʟᴇ: `/set_prefix @Team_TD_Links`**")
...
async def delete_prefix_handler(client, message):
    if client.name != "SnowRenamer":
        return
    if client.name != "SnowRenamer":
        return
...
async def see_prefix_handler(client, message):
    if client.name != "SnowRenamer":
        return
    if client.name != "SnowRenamer":
        return
